...
class SVTRNet(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.patch_merging is not None:
            ...
            # x = self.sub_sample2(x.permute(0, 2, 1).reshape(-1, self.embed_dim[1], self.HW[0], self.HW[1]))
    ...
    def forward(self, x):
        ...
        return x.permute(0, 2, 1).contiguous().view(n, e, h // 8, w // 8)


def svtr_tiny(img_size, pretrained=False):
    model = SVTRNet(
        img_size=img_size,
        out_char_num=25,
        out_channels=192,
        patch_merging='Conv',
        patch_merging_stride=[[2, 2], [1, 1]],
        embed_dim=[64, 128, 256],
        depth=[3, 6, 3],
        num_heads=[2, 4, 8],
        mixer=['Local', 'Local', 'Local', 'Local', 'Local', 'Local', 'Global', 'Global', 'Global', 'Global', 'Global',
               'Global'],
        local_mixer=[[7, 11], [7, 11], [7, 11]],
        last_stage=False,
        prenorm=False
    ...
    if pretrained:
        model.load_state_dict(torch.load('./workdir/svtr_tiny.pth'), strict=False)
    return model


def svtr_small(img_size, pretrained=False):
    model = SVTRNet(
        img_size=img_size,
        out_char_num=25,
        out_channels=192,
        ...
        depth=[3, 6, 6],
        num_heads=[3, 6, 8],
        mixer=['Local', 'Local', 'Local', 'Local', 'Local', 'Local', 'Local', 'Local', 'Global', 'Global', 'Global',
    ...
    if pretrained:
        model.load_state_dict(torch.load('./workdir/svtr_small.pth'), strict=False)
    return model


def svtr_base(img_size, pretrained=False):
...
if __name__ == '__main__':
    SVTR = svtr_base(img_size=[32, 256], pretrained=False)

    model_parameters = filter(lambda p: p.requires_grad, SVTR.parameters())

    print(f'Model created, trainable parameters: {sum([np.prod(p.size()) for p in model_parameters]) / 1000000.0} MB.')

    input = torch.randn(4, 3, 32, 256)
    output = SVTR(input)
    print(output.size())        # batch, h \times w, E
