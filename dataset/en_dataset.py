...
class LmdbDataset(Dataset):
    ...
    def __init__(self, lmdb_dir_root=None, transform=None, target_transform=None, img_w=256,
                 ...
                 convert_to_gray=True, max_length=25):

        self.en_characters = open("/path/to/util_files/en_keys.txt", 'r',
    ...
    def __get_images_and_labels(self):
        ...
        with self.env.begin(write=False) as txn:
            ...
            for i in range(nSamples):
                ...
                # We only train and evaluate on alphanumerics (or pre-defined character set)
                label = re.sub(f'[^{self.en_characters}]', '', label)
...
class CustomImagePreprocess:
    def __init__(self, _target_height, _target_width, _is_gray, _aug=False):
