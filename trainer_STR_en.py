...
class Trainer:
    ...
    def __init__(self, model, optimizer, config, data_loader, LabelTransformer,
        ...
        self.train_metrics_fine_prediction = AverageMetricTracker('fine_prediction_loss',
                                                                  writer=self.writer if self.local_master else None)
        self.train_metrics_coarse_prediction = AverageMetricTracker('coarse_prediction_loss',
                                                                    writer=self.writer if self.local_master else None)
        self.train_metrics_length_prediction = AverageMetricTracker('length_prediction_loss',
                                                                    writer=self.writer if self.local_master else None)

        self.train_metrics_ITC = AverageMetricTracker('teacher_student_ITC_loss',
                                                      writer=self.writer if self.local_master else None)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if epoch == self.start_epoch or epoch % self.validation_interval_epoch == 0 and epoch < self.validation_start_epoch:
                ...
                self.logger_info(
                    '[Epoch Start] Epoch:[{}/{}] LR: {:.8f}'.format(epoch, self.epochs, self._get_lr()) + val_res)
    ...
    def _train_epoch(self, epoch):
        ...
        for step_idx, input_data_item in enumerate(self.data_loader):
            ...
            with torch.autograd.set_detect_anomaly(self.config['trainer']['anomaly_detection']):
                ...
                coarse_logits, length_logits, fine_logits = outputs['coarse_logits'], outputs['length_logits'], outputs[
                    'fine_logits']

                vision_encoder_global_features, text_encoder_global_features = outputs['vision_encoder_features'], \
                outputs['text_encoder_features']

                loss_input = {'length': length_logits, 'coarse': coarse_logits, 'fine': fine_logits,
                              'vision_global_features': vision_encoder_global_features,
                              'text_global_features': text_encoder_global_features}
                loss_target = {'length': target_lengths, 'text': target}

                loss_fine, loss_coarse, loss_length, loss_ITC = get_loss(loss_input, loss_target, tau=
                self.config['model_arch']['args']['decoder_kwargs']['temperature'])

                loss = loss_fine + loss_coarse + loss_length + loss_ITC
    ...
    def _valid_epoch(self, epoch):
        ...
        for step_idx, input_data_item in enumerate(self.valid_data_loader):
            ...
            if self.distributed:
                word_acc, word_acc_case_ins, edit_distance_acc, total_distance_ref, batch_total = self._distributed_predict(
                    batch_size, images, text_label)
                ...
                with torch.no_grad():
                    ...
                    for index, (pred, text_gold) in enumerate(zip(fine_logits, text_label)):
                        ...
                        predict_text = re.findall(r'[a-zA-Z0-9]', predict_text.lower())
                        predict_text = ''.join(predict_text)

                        text_gold = re.findall(r'[a-zA-Z0-9]', text_gold.lower())
                        text_gold = ''.join(text_gold)
