...
import dataset.en_dataset as CustomDataset
from dataset.en_dataset import DistValSampler, DistCollateFn, LabelConverter

from parse_config import ConfigParser

import model_en as CustomModel

from trainer_STR_en import Trainer
...
def main(config: ConfigParser, local_master: bool, logger=None):
    ...
    train_dataset = config.init_obj('train_dataset', CustomDataset,
                                    transform=CustomDataset.CustomImagePreprocess(img_h, img_w, convert_to_gray, _aug=True),
                                    convert_to_gray=convert_to_gray)
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset) if config['distributed'] else None  # ImbalancedDatasetSampler(train_dataset)
    ...
    val_dataset = config.init_obj('val_dataset', CustomDataset,
                                  transform=CustomDataset.CustomImagePreprocess(img_h, img_w, convert_to_gray),
                                  convert_to_gray=convert_to_gray)

    val_sampler = DistValSampler(list(range(len(val_dataset))), batch_size=val_batch_size, distributed=config['distributed'])
    ...
    logger.info(f'Model created, trainable parameters: {model.model_parameters() / 1000000.0} MB.') if local_master else None

    # build optimizer, learning rate scheduler.
    optimizer = config.init_obj('optimizer', torch.optim, model.parameters())
    if config['lr_scheduler']['type'] is not None:
        if config['lr_scheduler']['type'] == 'OneCycleLR':
            config['lr_scheduler']['args']['max_lr'] = config['optimizer']['args']['lr']
            config['lr_scheduler']['args']['epochs'] = config['trainer']['epochs']
            config['lr_scheduler']['args']['steps_per_epoch'] = len(train_data_loader)
...
def parse_args():
    global config
    args = argparse.ArgumentParser(description='TEXT RECOGNIZER PyTorch Distributed Training')
    args.add_argument('-c', '--config',
                      # default='path/to/config_file.json',
