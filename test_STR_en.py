...
def _valid(config: ConfigParser, model, test_data_loader, device, LabelTransformer):
    ...
    for step_idx, input_data_item in enumerate(test_data_loader):
        ...
        with torch.no_grad():
            ...
            fine_logits, coarse_logits, length_logits, seq_decoder_features = predict(model, images, max_length=25)
            ...
            if total_edit_distance == total_distance_ref == 0:
                edit_distance_acc = 0.0
            else:
    ...
    print("\n This test using : {:.5f} ms per images \n".format((time.time() - start) / len(test_data_loader) * 1000))
