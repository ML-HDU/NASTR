...
class NASTR(nn.Module):
    ...
    def __init__(self, common_kwargs, encoder_kwargs, decoder_kwargs):
        ...
        if encoder_kwargs['type'] == 'vit':
            self.Encoder = VisionTransformerMAERec(img_size=(32, 256), patch_size=4, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True, pretrained=None)
            self.dimensions = 384
        elif encoder_kwargs['type'] == 'svtr_base':
        ...
        self.TransformerDecoder = Decoder(self.dimensions, self.n_head, dim_feedforward=4 * self.dimensions, dropout=self.dropout, num_layers=decoder_kwargs['stacks'])
    ...
    def forward(self, images, target_lengths, text_target=None):
        ...
        length_logits, length_preds = self.length_predictor(global_features)

        coarse_logits, coarse_preds = self.coarse_predictor(global_features, torch.min(torch.max(length_preds), torch.tensor(batch_max_length).to(images.device)))
