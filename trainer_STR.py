...
class Trainer:
    ...
    def __init__(self, model, optimizer, config, data_loader, LabelTransformer,
        ...
        self.validation_interval_epoch = self.config['trainer']['val_epoch_interval']
        ...
        self.log_step = log_step if log_step != -1 and 0 < log_step < self.len_step else int(np.sqrt(data_loader.batch_size))
        ...
        self.train_metrics = AverageMetricTracker('loss', writer=self.writer if self.local_master else None)
        self.train_metrics_fine_prediction = AverageMetricTracker('fine_prediction_loss', writer=self.writer if self.local_master else None)
        self.train_metrics_coarse_prediction = AverageMetricTracker('coarse_prediction_loss', writer=self.writer if self.local_master else None)
        self.train_metrics_length_prediction = AverageMetricTracker('length_prediction_loss', writer=self.writer if self.local_master else None)

        self.train_metrics_ITC = AverageMetricTracker('teacher_student_ITC_loss', writer=self.writer if self.local_master else None)

        self.val_metrics = AverageMetricTracker('loss', 'word_acc', 'word_acc_case_insensitive', 'edit_distance_acc', writer=self.writer if self.local_master else None)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            self.valid_data_loader.batch_sampler.set_epoch(epoch) if self.valid_data_loader.batch_sampler is not None else None
            
            if epoch == self.start_epoch or epoch % self.validation_interval_epoch == 0 and epoch < self.validation_start_epoch:
                val_metric_res_dict = self._valid_epoch(epoch)
                val_res = f"\nValidation result at {epoch} epoch: " \
                ...
                self.logger_info('[Epoch Start] Epoch:[{}/{}] LR: {:.8f}'.format(epoch, self.epochs, self._get_lr()) + val_res)
            ...
            self.logger_info(
                ...
                format(epoch, self.epochs, result_dict['loss'], self._get_lr()) + val_res
            )
            ...
            if best and (epoch % self.save_period == 0 and epoch >= self.validation_start_epoch):
    ...
    def _train_epoch(self, epoch):
        ...
        self.train_metrics_fine_prediction.reset()
        self.train_metrics_coarse_prediction.reset()
        self.train_metrics_length_prediction.reset()
        self.train_metrics_ITC.reset()
        ...
        for step_idx, input_data_item in enumerate(self.data_loader):
            ...
            target = self.LabelTransformer.encode(text_label)
            AR_target = self.LabelTransformer.AR_encode(text_label)
            target, AR_target = target.to(self.device), AR_target.to(self.device)
            target, AR_target = target.permute(1, 0), AR_target.permute(1, 0)

            target_lengths = self.LabelTransformer.get_length(target)

            with torch.autograd.set_detect_anomaly(self.config['trainer']['anomaly_detection']):

                outputs = self.model(images, target_lengths, AR_target)

                coarse_logits, length_logits, fine_logits = outputs['coarse_logits'], outputs['length_logits'], outputs['fine_logits']
                
                vision_encoder_global_features, text_encoder_global_features = outputs['vision_encoder_features'], outputs['text_encoder_features']

                loss_input = {'length': length_logits, 'coarse': coarse_logits, 'fine': fine_logits, 'vision_global_features': vision_encoder_global_features, 'text_global_features': text_encoder_global_features}
                loss_target = {'length': target_lengths, 'text': target}

                loss_fine, loss_coarse, loss_length, loss_ITC = get_loss(loss_input, loss_target, tau=self.config['model_arch']['args']['decoder_kwargs']['temperature'])

                loss = loss_fine + 0.1 * loss_length + loss_coarse + loss_ITC
            ...
            reduced_loss_fine = loss_fine.item()
            reduced_loss_coarse = loss_coarse.item()
            reduced_loss_length = loss_length.item()

            reduced_loss_ITC = loss_ITC.item()
            ...
            if self.distributed:
                ...
                reduced_metrics_tensor_fine = torch.tensor([batch_total, reduced_loss_fine]).float().to(self.device)
                reduced_metrics_tensor_coarse = torch.tensor([batch_total, reduced_loss_coarse]).float().to(self.device)
                reduced_metrics_tensor_length = torch.tensor([batch_total, reduced_loss_length]).float().to(self.device)

                reduced_metrics_tensor_ITC = torch.tensor([batch_total, reduced_loss_ITC]).float().to(self.device)
                ...
                reduced_metrics_tensor_fine = self.sum_tesnor(reduced_metrics_tensor_fine)
                batch_total, reduced_loss_fine = reduced_metrics_tensor_fine.cpu().numpy()
                reduced_loss_fine = reduced_loss_fine / dist.get_world_size()

                reduced_metrics_tensor_coarse = self.sum_tesnor(reduced_metrics_tensor_coarse)
                batch_total, reduced_loss_coarse = reduced_metrics_tensor_coarse.cpu().numpy()
                reduced_loss_coarse = reduced_loss_coarse / dist.get_world_size()

                reduced_metrics_tensor_length = self.sum_tesnor(reduced_metrics_tensor_length)
                batch_total, reduced_loss_length = reduced_metrics_tensor_length.cpu().numpy()
                reduced_loss_length = reduced_loss_length / dist.get_world_size()

                reduced_metrics_tensor_ITC = self.sum_tesnor(reduced_metrics_tensor_ITC)
                batch_total, reduced_loss_ITC = reduced_metrics_tensor_ITC.cpu().numpy()
                reduced_loss_ITC = reduced_loss_ITC / dist.get_world_size()
            ...
            self.train_metrics.update('loss', reduced_loss, batch_total)  # here, loss is mean results over batch, accumulate values
            self.train_metrics_fine_prediction.update('fine_prediction_loss', reduced_loss_fine, batch_total)
            self.train_metrics_coarse_prediction.update('coarse_prediction_loss', reduced_loss_coarse, batch_total)
            self.train_metrics_length_prediction.update('length_prediction_loss', reduced_loss_length, batch_total)

            self.train_metrics_ITC.update('teacher_student_ITC_loss', reduced_loss_ITC, batch_total)

            # log messages
            if step_idx % self.log_step == 0 or step_idx == 1:
                self.logger_info(
                    'Train Epoch:[{}/{}] Step:[{}/{}] Loss: {:.6f} Loss_avg: {:.6f} LR: {:.8f} '
                    'Loss Fine: {:.6f} Loss Coarse: {:.6f} Loss Length: {:.6f} Loss ITC: {:.6f}'.
                    format(epoch, self.epochs, step_idx, self.len_step,
                           self.train_metrics.val('loss'),
                           self.train_metrics.avg('loss'), self._get_lr(),
                           self.train_metrics_fine_prediction.val('fine_prediction_loss'),
                           self.train_metrics_coarse_prediction.val('coarse_prediction_loss'),
                           self.train_metrics_length_prediction.val('length_prediction_loss'),
                           self.train_metrics_ITC.val('teacher_student_ITC_loss')))
            ...
            if self.do_validation and step_idx % self.val_step_interval == 0 and epoch >= self.validation_start_epoch:
                ...
                best, not_improved_count = self._is_best_monitor_metric(False, 0, val_metric_res_dict, update_not_improved_count=False)
    ...
    def _valid_epoch(self, epoch):
        ...
        for step_idx, input_data_item in enumerate(self.valid_data_loader):
            ...
            if self.distributed:
                word_acc, word_acc_case_ins, edit_distance_acc, total_distance_ref, batch_total = self._distributed_predict(batch_size, images, text_label)
                ...
                with torch.no_grad():
                    ...
                    # (bs, max_len)
                    fine_logits, _, _, _ = predict(model, images, max_length=self.max_seq_len)
                    ...
                    for index, (pred, text_gold) in enumerate(zip(fine_logits, text_label)):
                        ...
                        for i in range(len(pred)):  # decode one sample
                            if pred[i] == self.LabelTransformer.EOS:
                                break

                            decoded_char = self.LabelTransformer.decode(pred[i])
